...
public class RelacionamentoDAO {
    ...
    public static Beneficiario buscarBeneficiario(Connection conexao, int id) throws SQLException {
        if (id <= 0) return null;
        try (PreparedStatement stmt = conexao.prepareStatement("SELECT * FROM beneficiario WHERE id_bene = ?")) {
            stmt.setInt(1, id);
    ...
    public static Dentista buscarDentista(Connection conexao, int id) throws SQLException {
        if (id <= 0) return null;
        try (PreparedStatement stmt = conexao.prepareStatement("SELECT * FROM dentista WHERE id_dent = ?")) {
            stmt.setInt(1, id);
            try (ResultSet rs = stmt.executeQuery()) {
                if (!rs.next()) return null;
    ...
    public static IntegranteTDB buscarIntegrante(Connection conexao, int id) throws SQLException {
        if (id <= 0) return null;
        try (PreparedStatement stmt = conexao.prepareStatement("SELECT * FROM integrante_tdb WHERE id_integ = ?")) {
            stmt.setInt(1, id);
            try (ResultSet rs = stmt.executeQuery()) {
                if (!rs.next()) return null;
    ...
    public static Patrocinador buscarPatrocinador(Connection conexao, int id) throws SQLException {
        ...
        try (PreparedStatement stmt = conexao.prepareStatement("SELECT * FROM patrocinador WHERE id_patr = ?")) {
            stmt.setInt(1, id);
            try (ResultSet rs = stmt.executeQuery()) {
                if (!rs.next()) return null;
    ...
    public static Caso buscarCaso(Connection conexao, int id) throws SQLException {
        if (id <= 0) return null;
        try (PreparedStatement stmt = conexao.prepareStatement("SELECT * FROM caso WHERE id_caso = ?")) {
            stmt.setInt(1, id);
            try (ResultSet rs = stmt.executeQuery()) {
                if (!rs.next()) return null;
    ...
    public static Caso montarCaso(Connection conexao, ResultSet rs) throws SQLException {
        ...
        c.setBeneficiario(buscarBeneficiario(conexao, rs.getInt("fk_beneficiario_id_bene")));
        int idDentista = rs.getInt("fk_dentista_id_dent");
        if (!rs.wasNull()) c.setDentista(buscarDentista(conexao, idDentista));
        c.setIntegrante(buscarIntegrante(conexao, rs.getInt("fk_integrante_id_integ")));
        c.setTemDiagnostico(existeDiagnostico(conexao, c.getIdCaso()));
        return c;
    }

    public static boolean existeDiagnostico(Connection conexao, int idCaso) throws SQLException {
        try (PreparedStatement stmt = conexao.prepareStatement("SELECT COUNT(*) FROM diagnostico WHERE fk_caso_id_caso = ?")) {
