...
public class IntegranteTDBDAO {
    ...
    public void deletar(int id) throws SQLException, ClassNotFoundException {
        try (Connection conexao = abrirConexao();
             PreparedStatement stmt = conexao.prepareStatement("DELETE FROM integrante_tdb WHERE id_integ=?")) {
            stmt.setInt(1, id);
            stmt.executeUpdate();
        }
    }
    ...
    public IntegranteTDB buscarPorId(int id) throws SQLException, ClassNotFoundException {
        try (Connection conexao = abrirConexao();
             PreparedStatement stmt = conexao.prepareStatement("SELECT * FROM integrante_tdb WHERE id_integ=?")) {
            stmt.setInt(1, id);
    ...
    public List<IntegranteTDB> selecionar() throws SQLException, ClassNotFoundException {
        ...
        try (Connection conexao = abrirConexao();
             PreparedStatement stmt = conexao.prepareStatement("SELECT * FROM integrante_tdb ORDER BY id_integ");
