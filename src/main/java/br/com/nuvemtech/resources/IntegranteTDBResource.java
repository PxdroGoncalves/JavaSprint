package br.com.nuvemtech.resources;

import br.com.nuvemtech.dto.LoginRequest;
import br.com.nuvemtech.services.IntegranteTDBService;
import br.com.nuvemtech.entities.IntegranteTDB;
import jakarta.ws.rs.*;
import jakarta.ws.rs.core.*;

import java.sql.SQLException;
import java.util.List;

@Path("/integrantes")
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
public class IntegranteTDBResource {

    private final IntegranteTDBService service = new IntegranteTDBService();

    @GET
    public List<IntegranteTDB> selecionar() throws SQLException, ClassNotFoundException {
        return service.selecionar();
    }

    @GET
    @Path("/{id}")
    public IntegranteTDB buscarPorId(@PathParam("id") int id) throws SQLException, ClassNotFoundException {
        return service.buscarPorId(id);
    }

    @POST
    public Response inserir(IntegranteTDB obj, @Context UriInfo uriInfo) throws SQLException, ClassNotFoundException {
        service.inserir(obj);
        UriBuilder builder = uriInfo.getAbsolutePathBuilder();
        builder.path(Integer.toString(obj.getIdIntegrante()));
        return Response.created(builder.build()).entity(obj).build();
    }

    @POST
    @Path("/cadastrar")
    public Response cadastrar(IntegranteTDB obj) throws SQLException, ClassNotFoundException {
        service.cadastrar(obj);
        return Response.status(Response.Status.CREATED).entity(obj).build();
    }

    @PUT
    @Path("/{id}")
    public Response atualizar(@PathParam("id") int id, IntegranteTDB obj) throws SQLException, ClassNotFoundException {
        obj.setIdIntegrante(id);
        service.atualizar(obj);
        return Response.ok(obj).build();
    }

    @DELETE
    @Path("/{id}")
    public Response deletar(@PathParam("id") int id) throws SQLException, ClassNotFoundException {
        service.deletar(id);
        return Response.noContent().build();
    }

    @POST
    @Path("/login")
    public Response login(LoginRequest request) throws SQLException, ClassNotFoundException {
        IntegranteTDB obj = service.login(request.getEmail(), request.getSenha());
        return Response.ok(obj).build();
    }
}
