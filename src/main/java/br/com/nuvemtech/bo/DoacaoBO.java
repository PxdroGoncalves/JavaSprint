...
public class DoacaoBO {
    public List<Doacao> selecionarBo() throws SQLException, ClassNotFoundException { return new DoacaoDAO().selecionar(); }
    ...
    public void deletarBo(int id) throws SQLException, ClassNotFoundException { new DoacaoDAO().deletar(id); }
    private void validar(Doacao d) {
        ...
        if (d.getTipo().equalsIgnoreCase("MONETARIO") && (d.getValor() == null || d.getValor() <= 0)) throw new RegraNegocioException("Doacao monetaria precisa ter valor maior que zero.");
        if (d.getTipo().equalsIgnoreCase("EQUIPAMENTO") && (d.getDescricaoEquipamento() == null || d.getDescricaoEquipamento().isBlank())) throw new RegraNegocioException("Doacao de equipamento precisa ter descricao.");
        if (d.getPatrocinador() == null || d.getPatrocinador().getIdPatrocinador() <= 0) throw new RegraNegocioException("Patrocinador obrigatorio.");
    }
}
