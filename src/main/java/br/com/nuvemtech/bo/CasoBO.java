...
public class CasoBO {
    ...
    public void deletarBo(int id) throws SQLException, ClassNotFoundException {
        if (id <= 0) throw new RegraNegocioException("ID do caso invalido.");
    ...
    public List<Caso> listarTodosBo() throws SQLException, ClassNotFoundException {
    ...
    private void validarInsercao(Caso caso) {
        ...
        if (caso.getBeneficiario() == null || caso.getBeneficiario().getIdBeneficiario() <= 0)
            throw new RegraNegocioException("Beneficiario obrigatorio.");
        if (caso.getIntegrante() == null || caso.getIntegrante().getIdIntegrante() <= 0)
            throw new RegraNegocioException("Integrante obrigatorio.");
        
        if (caso.getStatus() != null && !caso.getStatus().isBlank()) {
            java.util.Set<String> validos = java.util.Set.of("PENDENTE","EM_ANDAMENTO","CONCLUIDO","CANCELADO");
            if (!validos.contains(caso.getStatus()))
